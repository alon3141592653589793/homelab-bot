const entry = {
  ...
  description: "One-time setup that spins up a WireGuard VPN on the Pi via Docker (wg-easy). Checks docker + docker-group membership, ensures net.ipv4.ip_forward, writes wireguard/{.env,docker-compose.yml} (WG_HOST / WG_PASSWORD / WG_DEFAULT_DNS), opens ufw 51820/51821 if active, and runs 'docker compose up -d'. Run manually on the Pi with flags (--host/--password/--dns) or env (WG_HOST/WG_PASSWORD/WG_DEFAULT_DNS), else prompts interactively. Prints web UI URL + reminders (router port forwarding, bot channel env vars).",
