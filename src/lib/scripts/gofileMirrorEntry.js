const entry = {
  ...
  description: "Cloud-to-cloud mirror of a HuggingFace model file -> Gofile, designed as an anti-censorship backup (if HF takes the file down, the verified Gofile copy survives). The model is NEVER written to the Pi's SD card: it streams into a tmpfs staging dir (/dev/shm), is sha256-verified against HF's published LFS hash, uploaded to Gofile, cross-checked against Gofile's returned md5, then the staging is wiped. Only a tiny manifest (content id, share code, sha256, size) persists on the Pi. Source download is RESUMABLE via HTTP Range so a network drop at 40GB doesn't restart from 0; a detected bit flip (whole-file sha mismatch) triggers a full re-download (HF only publishes whole-file sha256, so per-chunk resume-on-flip isn't possible). Defaults to prajjwal1/bert-tiny (~17MB) so you can prove the pipeline end-to-end first.",
...
def resumable_download(url, dest):
    ...
    RAM stays ~1MB. Returns (sha256, md5, size). On resume we re-hash the
    existing partial so the final hash covers start..end correctly."""
    sha = hashlib.sha256()
    md5 = hashlib.md5()
    have = 0
